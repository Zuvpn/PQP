...
generate_config(){
  ...
  _write_yaml(){
    #
    # paqet config schema (newer builds):
    # - requires a network interface name + at least one address family (ipv4/ipv6)
    # - expects transport as a mapping: transport.protocol + transport.kcp
    # - expects listen/remote/socks as mappings with an "addr" key
    #
    local out="$1" role="$2" key="$3" listen="$4" remote="$5" socks="$6"

    # Prevent `set -u` crashes.
    logl="${logl:-${LOG_LEVEL:-info}}"

    # Detect interface from default route if not already exported.
    IFACE="${IFACE:-}"
    if [[ -z "$IFACE" ]]; then
...
YAML
  ...
  if [[ "$ROLE" == "server" ]]; then
    ...
    for p in $ports; do
      _write_yaml "/etc/paqet/paqet_${p}.yaml" "server" "$KEY" "0.0.0.0:${p}" "" "0"
    done
  else
    # client instance uses SERVER_PORT (tunnel port) as %i
    _write_yaml "/etc/paqet/paqet_${SERVER_PORT}.yaml" "client" "$KEY" ":0" "${SERVER_IP}:${SERVER_PORT}" "${SOCKS_PORT}"
  fi
