...
ask() {
  ...
  # Robust against non-interactive stdin: reads from /dev/tty if available
  local prompt="${1:-}"
  local def="${2:-}"
  local ans=""
  ...
  # Prefer direct terminal
  if [[ -t 0 ]]; then
    read -r -p "$prompt" ans || ans=""
  elif [[ -r /dev/tty ]]; then
    # When run via pipe/automation, stdin is not interactive; read from controlling tty
    read -r -p "$prompt" ans < /dev/tty || ans=""
  else
    # Non-interactive environment with no tty: return default or empty
    ans=""
  fi
...
# Remove forward rules that conflict with SOCKS port (prevents socat from stealing SOCKS port)
sanitize_forwards(){
  local forwards_csv="$1" socks_port="$2" out="" item lport rest
  [[ -n "$forwards_csv" ]] || { echo ""; return 0; }
  IFS=',' read -r -a _items <<<"$forwards_csv"
  for item in "${_items[@]}"; do
    [[ -n "$item" ]] || continue
    lport="${item%%:*}"
    rest="${item#*:}"
    # skip invalid or conflicting
    [[ "$lport" =~ ^[0-9]+$ ]] || continue
    if [[ -n "$socks_port" && "$lport" == "$socks_port" ]]; then
      continue
    fi
    out+="${lport}:${rest},"
  done
...
stop_disable_forward_port(){
  local p="$1"
  [[ -n "$p" ]] || return 0
  systemctl stop "pqp-forward@${p}.service" 2>/dev/null || true
  systemctl disable "pqp-forward@${p}.service" 2>/dev/null || true
  # Prevent auto-restart / future enable from stealing the port again (can be unmasked later)
  systemctl mask "pqp-forward@${p}.service" 2>/dev/null || true
}

unmask_enable_forward_port(){
  local p="$1"
  [[ -n "$p" ]] || return 0
  systemctl unmask "pqp-forward@${p}.service" 2>/dev/null || true
  systemctl enable --now "pqp-forward@${p}.service" 2>/dev/null || true
}

ensure_port_free_for_socks(){
  # Ensure SOCKS port isn't occupied by pqp-forward (socat) or anything else.
  # If occupied by pqp-forward/socat, stop/disable/mask it.
  local socks_port="$1"
  [[ -n "$socks_port" ]] || return 0

  local line proc pid
  line="$(ss -lntp 2>/dev/null | awk -v p=":${socks_port}" '$4 ~ p"$" {print; exit}')"
  [[ -n "$line" ]] || return 0

  # Extract process name and pid if possible
  proc="$(echo "$line" | sed -n 's/.*users:(("([^"]+)".*/\1/p' | head -n1)"
  pid="$(echo "$line" | sed -n 's/.*pid=\([0-9][0-9]*\).*/\1/p' | head -n1)"

  if [[ "$proc" == "socat" ]]; then
    stop_disable_forward_port "$socks_port"
...
normalize_ports(){
  # Accepts: "9999", "9999,8888", "9999 8888", "9999, 8888"
  # Outputs a comma-separated unique list, or empty on invalid input.
  local in="${1:-}"
  in="${in// /,}"
  in="${in//,,/,}"
  in="${in#,}"; in="${in%,}"
  ...
  local out=() seen="," tok p
  IFS=',' read -r -a tokarr <<< "$in"
  for tok in "${tokarr[@]}"; do
    tok="${tok//[[:space:]]/}"
    [[ -n "$tok" ]] || continue
    [[ "$tok" =~ ^[0-9]+$ ]] || { echo ""; return 1; }
    p="$tok"
    (( p>=1 && p<=65535 )) || { echo ""; return 1; }
    if [[ "$seen" != *",$p,"* ]]; then
      out+=("$p")
      seen+="$p,"
    fi
  done
  [[ "${#out[@]}" -gt 0 ]] || { echo ""; return 1; }
  (IFS=','; echo "${out[*]}")
}
...
# Paqet official schema (transport.kcp.*)
KCP_BLOCK=aes
# KCP_KEY is derived from KEY by default (sha256 hex) for both sides
KCP_KEY=
EOF
  chmod 600 "$ENV_FILE"
}
...
derive_kcp_key(){
  # Deterministic KCP key derived from KEY so client/server always match.
  # Output: 64-char hex sha256.
  local base="${1:-}"
  [[ -n "$base" && "$base" != "CHANGE_ME" ]] || return 1
...
detect_iface(){
  ...
  [[ -n "$iface" ]] || iface="$(ip -o link show 2>/dev/null | awk -F': ' '!/ lo:/{print $2; exit}' | sed 's/@.*//')"
  echo "${iface:-eth0}"
}
...
detect_router_mac(){
  # Best-effort router MAC discovery (required by paqet).
  # Tries multiple strategies + retries to handle noisy neighbor tables.
  local iface="$1" gw="$2" mac=""
  [[ -n "$iface" && -n "$gw" ]] || return 1

  # Retry a few times (neighbor table can be empty right after boot)
  local i
  for i in 1 2 3; do
    ping -c1 -W1 "$gw" >/dev/null 2>&1 || true
    mac="$(ip neigh show to "$gw" dev "$iface" 2>/dev/null | awk '{print $5; exit}')"
    [[ -n "$mac" && "$mac" != "FAILED" ]] && break
    sleep 0.2
  done

  if [[ -z "$mac" || "$mac" == "FAILED" ]]; then
    if ! have arping; then
      (apt-get update -y >/dev/null 2>&1 && apt-get install -y arping >/dev/null 2>&1) || true
    fi
    if have arping; then
      timeout 2 arping -I "$iface" -c 2 "$gw" >/dev/null 2>&1 || true
      mac="$(ip neigh show to "$gw" dev "$iface" 2>/dev/null | awk '{print $5; exit}')"
    fi
  fi

  if [[ -z "$mac" || "$mac" == "FAILED" ]]; then
    mac="$(ip neigh show dev "$iface" 2>/dev/null | awk '/ router /{for(i=1;i<=NF;i++) if($i=="lladdr"){print $(i+1); exit}}')"
  fi

  [[ -n "$mac" && "$mac" != "FAILED" ]] || return 1
...
wizard_create_tunnel(){
  ...
  while true; do
    ...
    case "$sel" in
      1) ROLE="client"; break ;;
      2) ROLE="server"; break ;;
      *) if [[ -z "${sel:-}" ]]; then echo "Waiting for input... (if you used a pipe, run: sudo pqp)"; sleep 0.2; else echo "Please enter 1 or 2."; fi ;;
    esac
  done
...
generate_config(){
  ...
  mtu="$(pick_mtu)"; mode="${KCP_MODE:-fast}"; conn="${KCP_CONN:-1}"; rcv="${KCP_RCVWND:-512}"; snd="${KCP_SNDWND:-512}"
    ...
    # Optional port forwarding (skip by pressing Enter)
    irports="$(ask "IRAN_IN_PORTS (optional, comma-separated) ex: 443,8443,2053 (Enter to skip)" "${IR_LISTEN_PORTS:-}")"
    ir_norm=""
    if [[ -n "${irports//[[:space:]]/}" ]]; then
      ir_norm="$(normalize_ports "$irports" 2>/dev/null || true)"
      [[ -n "$ir_norm" ]] || die "Invalid IRAN_IN_PORTS. Example: 443,8443,2053"
    fi
    ...
    if [[ -n "$ir_norm" ]]; then
      ...
      dest_port="${FWD_DEST_PORT:-}"
      dest_host="${FWD_DEST_HOST:-}"
    fi
  ...
  # ---- prevent SOCKS/forward port conflict ----
  if [[ -n "${SOCKS_PORT:-}" && -n "${FORWARDS:-}" ]]; then
    local _san
    _san="$(sanitize_forwards "$FORWARDS" "$SOCKS_PORT")"
    if [[ "$_san" != "$FORWARDS" ]]; then
      echo "[!] Removed forward rule(s) that conflicted with SOCKS_PORT=${SOCKS_PORT}" >&2
      FORWARDS="$_san"
      save_env_kv "FORWARDS" "$FORWARDS"
    fi
    # If a forward service already exists on SOCKS_PORT, stop/disable/mask it.
    stop_disable_forward_port "$SOCKS_PORT"
  fi

  # Also ensure the SOCKS port is free right now (common issue: socat still holds it)
  ensure_port_free_for_socks "${SOCKS_PORT:-}"

  # ------------------------------------------------------------------
  # IMPORTANT: systemd unit runs:
  #   /usr/local/bin/paqet run -c /etc/paqet/paqet_%i.yaml
  # So we must actually write those YAML files; otherwise services will
  # endlessly restart with "no such file or directory".
  # ------------------------------------------------------------------
  mkdir -p /etc/paqet
  umask 077

  # ---- detect network details required by paqet examples (router_mac is REQUIRED) ----
  local IFACE GW IP ROUTER_MAC
  IFACE="$(detect_iface)"
  IP="$(detect_ipv4 "$IFACE")"
  GW="$(detect_gw)"
  ROUTER_MAC="${ROUTER_MAC:-}"
  if [[ -z "$ROUTER_MAC" && -n "$GW" ]]; then
    ROUTER_MAC="$(detect_router_mac "$IFACE" "$GW" 2>/dev/null || true)"
  fi

  if [[ -z "$IP" ]]; then
    echo "[!] Could not detect IPv4 address for interface $IFACE" >&2
  fi

  if [[ -z "$GW" ]]; then
    echo "[!] Could not detect default gateway (required)." >&2
  fi

  if [[ -z "$ROUTER_MAC" ]]; then
    echo "[!] Could not detect router MAC automatically (required by paqet)." >&2
    echo "    You can get it via:" >&2
    echo "      ip route show default" >&2
    echo "      ip neigh show dev $IFACE" >&2
    if is_tty; then
      ROUTER_MAC="$(ask "Enter router MAC (example: aa:bb:cc:dd:ee:ff)" "")"
    fi
  fi

  [[ -n "$ROUTER_MAC" ]] || die "Router MAC is required. Could not auto-detect. Run: ip neigh show dev $IFACE (gateway line)"
  save_env_kv "ROUTER_MAC" "$ROUTER_MAC"

  # Derive deterministic KCP_KEY from KEY so both sides always match.
  # User can still override by manually setting KCP_KEY in /etc/pqp/pqp.env
  local derived
  derived="$(derive_kcp_key "$KEY" 2>/dev/null || true)"
  if [[ -n "$derived" ]]; then
    KCP_KEY="$derived"
    save_env_kv "KCP_KEY" "$KCP_KEY"
  fi

  _write_yaml(){
    # ==================================================================
    # Paqet config schema (official examples):
    #  Client:
    #    role, log.level, socks5[], network.interface, network.ipv4.addr,
    #    network.ipv4.router_mac, server.addr, transport.protocol,
    #    transport.kcp.block, transport.kcp.key, forward[] (optional)
    #
    #  Server:
    #    role, log.level, listen.addr, network.interface, network.ipv4.addr,
    #    network.ipv4.router_mac, transport.protocol, transport.kcp.block/key
    # ==================================================================
    local out="$1" role="$2" iface="$3" ipv4_addr="$4" router_mac="$5" \
          listen_addr="${6:-}" server_addr="${7:-}" socks_port="${8:-1080}" \
          kcp_block="${9:-aes}" kcp_key="${10:-}" forwards_csv="${11:-}"

    local lvl="${LOG_LEVEL:-info}"
    [[ -n "$kcp_key" ]] || kcp_key="${KCP_KEY:-}"
    [[ -n "$kcp_key" ]] || kcp_key="$KEY"  # last resort
...
YAML
      ...
      if [[ -n "$forwards_csv" ]]; then
        # forwards_csv format: inport:dest_host:dest_port,inport2:dest_host:dest_port
        local item inport host dport
        echo "forward:" >>"$out"
        IFS=',' read -r -a _items <<<"$forwards_csv"
        for item in "${_items[@]}"; do
          [[ -n "$item" ]] || continue
...
YAML
  ...
  if [[ "$ROLE" == "server" ]]; then
    local p ports listen_addr
    ports="$(parse_list "${LISTEN_PORT:-9999}")"
    for p in $ports; do
      listen_addr=":${p}"
      _write_yaml "/etc/paqet/paqet_${p}.yaml" "server" "$IFACE" "${IP}:${p}" "$ROUTER_MAC" "$listen_addr" "" "" "${KCP_BLOCK:-aes}" "${KCP_KEY:-$KEY}" ""
    done
  else
    # client instance uses SERVER_PORT (tunnel port) as %i
    _write_yaml "/etc/paqet/paqet_${SERVER_PORT}.yaml" "client" "$IFACE" "${IP}:0" "$ROUTER_MAC" "" "${SERVER_IP}:${SERVER_PORT}" "${SOCKS_PORT}" "${KCP_BLOCK:-aes}" "${KCP_KEY:-$KEY}" "${FORWARDS:-}"
  fi
...
start_all(){
  ...
  if [[ "$ROLE" == "server" ]]; then
    ...
    # Prevent forward services from stealing SOCKS port
    if [[ -n "${SOCKS_PORT:-}" ]]; then
      stop_disable_forward_port "$SOCKS_PORT"
    fi
    if [[ -n "${FORWARDS:-}" ]]; then
      FORWARDS="$(sanitize_forwards "$FORWARDS" "${SOCKS_PORT:-}")"
      save_env_kv "FORWARDS" "$FORWARDS"
    fi
