...
generate_config(){
  ...
  if [[ -n "$GW" ]]; then
    ...
    ROUTER_MAC="$(ip neigh show "$GW" dev "$IFACE" 2>/dev/null | awk '{print $5; exit}')"
  fi

  if [[ -z "$IP" ]]; then
    echo "[!] Could not detect IPv4 address for interface $IFACE" >&2
  fi
  if [[ -z "$GW" || -z "$ROUTER_MAC" || "$ROUTER_MAC" == "FAILED" ]]; then
    echo "[!] Could not detect router MAC (required)." >&2
    echo "    Try: ip route show default ; ip neigh show dev $IFACE" >&2
  fi

  # Ensure KCP_KEY exists (used by official schema: transport.kcp.key)
  if [[ -z "${KCP_KEY:-}" ]]; then
    if have "$PAQET_BIN"; then
      KCP_KEY="$($PAQET_BIN secret 2>/dev/null | head -n1 || true)"
      [[ -n "$KCP_KEY" ]] && save_env_kv "KCP_KEY" "$KCP_KEY"
    fi
  fi
