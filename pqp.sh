...
auto_detect_mtu_once(){
  ...
  local lo="${MTU_MIN_PAYLOAD:-1200}" hi="${MTU_MAX_PAYLOAD:-1472}" verify="${MTU_VERIFY:-5}" best="$lo"
  if ! ping_df_ok "$target" 900; then
    local fb tuned; fb="$(fallback_mtu "$target")"; tuned=$(( fb - margin ))
    (( tuned < 1200 )) && tuned=1200; (( tuned > 1450 )) && tuned=1450
    echo "$tuned"; return 0
  fi
  while [[ $lo -le $hi ]]; do
    local mid=$(( (lo + hi) / 2 ))
    if ping_df_ok "$target" "$mid"; then best="$mid"; lo=$(( mid + 1 )); else hi=$(( mid - 1 )); fi
  done
  local stable="$best"
...
median_of(){
  mapfile -t arr; local n="${#arr[@]}"; [[ $n -gt 0 ]] || return 1
  IFS=$'
' arr=($(printf "%s
" "${arr[@]}" | sort -n))
  [[ $n -eq 1 ]] && echo "${arr[0]}" && return 0
  [[ $n -eq 2 ]] && echo "${arr[0]}" && return 0
  echo "${arr[1]}"
}
...
pick_mtu(){
  load_env
  if [[ "${AUTO_MTU:-1}" != "1" ]]; then apply_mtu_cap "${MTU:-1350}"; return 0; fi
  local runs="${MTU_RUNS:-1}"; [[ "$runs" =~ ^[0-9]+$ ]] || runs=1
  (( runs < 1 )) && runs=1; (( runs > 3 )) && runs=3
  local target margin; target="$(get_mtu_target)"; margin="${MTU_MARGIN:-90}"
  local results=() i
  for ((i=1;i<=runs;i++)); do results+=("$(auto_detect_mtu_once "$target" "$margin")"); sleep 1; done
  local final
  if [[ "${MTU_MEDIAN:-1}" == "1" && ${#results[@]} -ge 2 ]]; then final="$(printf "%s
" "${results[@]}" | median_of)"; else final="${results[-1]}"; fi
  apply_mtu_cap "$final"
}
...
install_paqet(){
  ...
  tag="$(curl -fsSI https://github.com/hanselime/paqet/releases/latest | awk -F': ' 'tolower($1)=="location"{print $2}' | tr -d '' | tail -n1)"
  tag="${tag##*/}"; [[ -n "$tag" ]] || die "Failed to resolve latest paqet release tag"
  url="https://github.com/hanselime/paqet/releases/download/${tag}/paqet_linux_${arch}.tar.gz"
  mkdir -p "$WORKDIR"; tmp="$(mktemp -d)"
  curl -fL --connect-timeout 10 --max-time 180 "$url" -o "$tmp/paqet.tgz" || die "Download failed"
  tar -xzf "$tmp/paqet.tgz" -C "$WORKDIR"; rm -rf "$tmp"
  local bin; [[ -f "$WORKDIR/paqet" ]] && bin="$WORKDIR/paqet" || bin="$(find "$WORKDIR" -maxdepth 3 -type f -name paqet | head -n1 || true)"
  [[ -n "${bin:-}" && -f "$bin" ]] || die "paqet binary not found"
  chmod +x "$bin"; ln -sf "$bin" "$PAQET_BIN"
...
generate_config(){
  ...
  if [[ "$ROLE" == "server" ]]; then
    local ports; ports="$(parse_list "${LISTEN_PORT:-}")"; [[ -n "$ports" ]] || die "LISTEN_PORT required"
    for p in $ports; do
...
transport:
  protocol: "kcp"
  conn: ${conn}
  kcp: { mode: "${mode}", mtu: ${mtu}, rcvwnd: ${rcv}, sndwnd: ${snd}, key: "${KEY}" }
EOF
    done
  else
    local ports; ports="$(parse_list "${SERVER_PORT:-}")"; [[ -n "$ports" ]] || die "SERVER_PORT required"
    [[ -n "${SERVER_IP:-}" ]] || die "SERVER_IP required"
    for p in $ports; do
      cat >"${CFG_DIR}/paqet_${p}.yaml" <<EOF
role: "client"
log: { level: "${LOG_LEVEL}" }
